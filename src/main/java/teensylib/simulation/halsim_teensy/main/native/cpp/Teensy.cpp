...
Teensy::Teensy() : m_gyro_name { "TeensyGyro" }, m_wpilib_update_func(
		[](const wpi::json&) {
		}) {
	...
	m_encoder_inputs.emplace(4, 0);
	m_encoder_inputs.emplace(5, 0);
	m_encoder_inputs.emplace(6, 0);
	m_encoder_inputs.emplace(7, 0);
}

void Teensy::HandleWPILibUpdate(const wpi::json &data) {
...
void Teensy::HandleTeensyUpdate(std::span<const uint8_t> packet) {
	...
	while (!packet.empty()) {
		...
		switch (packet[1]) {
		case TEENSY_TAG_GYRO:
			ReadGyroTag(tagPacket);
			break;
		case TEENSY_TAG_DIO:
			ReadDIOTag(tagPacket);
			break;
		case TEENSY_TAG_ENCODER:
			ReadEncoderTag(tagPacket);
			break;
		case TEENSY_TAG_ANALOG:
...
void Teensy::SetupTeensySendBuffer(wpi::raw_uv_ostream &buf) {
	SetupSendHeader(buf);
	SetupMotorTag(buf);
	SetupServoTag(buf);
...
// WPILib Sim Handlers
void Teensy::HandleDriverStationSimValueChanged(const wpi::json &data) {
...
void Teensy::HandleMotorSimValueChanged(const wpi::json &data) {
	int deviceId = -1;
	auto motorData = data["data"];

	if (data["device"] == "motorFLD") {
		deviceId = 0;
	} else if (data["device"] == "motorFLA") {
		deviceId = 1;
	} else if (data["device"] == "motorRLD") {
		deviceId = 2;
	} else if (data["device"] == "motorRLA") {
		deviceId = 3;
	} else if (data["device"] == "motorRRD") {
		deviceId = 4;
	} else if (data["device"] == "motorRRA") {
		deviceId = 5;
	} else if (data["device"] == "motorFRD") {
		deviceId = 6;
	} else if (data["device"] == "motorFRA") {
		deviceId = 7;
	}
...
void Teensy::HandleDIOSimValueChanged(const wpi::json &data) {
	int deviceId = -1;
...
void Teensy::HandleGyroSimValueChanged(const wpi::json &data) {
	m_gyro_name = data["device"].get<std::string>();
}

//TODO: figure out how to properly do encoders
void Teensy::HandleEncoderSimValueChanged(const wpi::json &data) {
...
// ==================================
// Teensy Buffer Generation/Read Methods
// ==================================

void Teensy::SetupSendHeader(wpi::raw_uv_ostream &buf) {
	uint8_t pktSeq[2];
	wpi::support::endian::write16be(pktSeq, m_teensy_bound_seq);

	buf << pktSeq[0] << pktSeq[1]
			<< static_cast<uint8_t>(m_robot_enabled ? 1 : 0);
}

//TODO: checkout motor tag size, I believe this should be fine
void Teensy::SetupMotorTag(wpi::raw_uv_ostream &buf) {
	uint8_t value[6];

	for (auto motor : m_motor_outputs) {
		// Motor payload is 6 bytes
		buf << static_cast<uint8_t>(6)              // Size
				<< static_cast<uint8_t>(TEENSY_TAG_MOTOR)  // Tag
...
void Teensy::SetupDigitalOutTag(wpi::raw_uv_ostream &buf) {
	for (auto digitalOut : m_digital_outputs) {
		// DIO payload is 3 bytes
		buf << static_cast<uint8_t>(3)                           // Size
				<< static_cast<uint8_t>(TEENSY_TAG_DIO)                 // Tag
				<< static_cast<uint8_t>(digitalOut.first)            // Channel
				<< static_cast<uint8_t>(digitalOut.second ? 1 : 0);  // Value
	}
}

void Teensy::ReadGyroTag(std::span<const uint8_t> packet) {
	if (packet.size() < 26) {
		return;  // size(1) + tag(1) + 6x 4byte
	}

	packet = packet.subspan(2);  // Skip past the size and tag
	float rate_x = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[0]));
	float rate_y = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[4]));
	float rate_z = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[8]));
	float angle_x = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[12]));
	float angle_y = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[16]));
	float angle_z = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[20]));
...
void Teensy::ReadDIOTag(std::span<const uint8_t> packet) {
	if (packet.size() < 4) {
...
//TODO: figure out encoder tag length
void Teensy::ReadEncoderTag(std::span<const uint8_t> packet) {
	if (packet.size() < 7) {
	...
	int32_t count = static_cast<int32_t>(wpi::support::endian::read32be(
			&packet[0]));
	...
	if (containsPeriod) {
		// Older versions of Teensy firmware do not provide Encoder Period.
		...
		double period = static_cast<double>(period_numerator >> 1)
				/ period_denominator;
...
void Teensy::ReadAnalogTag(std::span<const uint8_t> packet) {
	if (packet.size() < 7) {
	...
	packet = packet.subspan(3);
	float voltage = wpi::bit_cast<float>(
			wpi::support::endian::read32be(&packet[0]));
